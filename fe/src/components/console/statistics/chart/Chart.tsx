...
const RechartsComponents = dynamic(
                ...
                <mod.ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        <mod.Legend
                            iconType="circle"
                            onClick={props => {
                                onLegendClick(props.dataKey as string);
                            }}
                        />
                        {datasets.map((dataset: ChartDataset, index: number) => (
                            <mod.Line
                                key={dataset.label}
        ...
        })),
    {
        ssr: false,
        loading: () => (
            <div className="flex h-full items-center justify-center text-white">차트 로딩중...</div>
        ),
    },
);
...
function Chart() {
    ...
    // API 데이터를 Recharts 형식으로 변환 (memoized)
