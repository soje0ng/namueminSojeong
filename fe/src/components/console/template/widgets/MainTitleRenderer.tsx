...
import { useWidgetStyle, getElementStyle, SafeHtml, WidgetRendererProps } from './WidgetUtils';

// 💡 [기본 폰트 사이즈 설정 안내]
// 이 영역의 값을 수정하면 메인 타이틀 위젯이 처음 추가될 때의 기본 크기가 변경됩니다.
// - PC 버전: fontSize: "80px"
// - 모바일 버전: fontSizeMobile: "40px"
        ...
        <section style={style} className="w-full h-auto">
            <div className="mx-auto w-full max-w-[1920px] text-center">
                ...
                <SafeHtml
                    ...
                    className="hover:outline-dashed hover:outline-2 hover:outline-blue-400 rounded transition-all inline-block font-bold text-[#111111] leading-tight"
                    style={getElementStyle(w.data.mainTitleStyle, viewport as any)}
                    onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('mainTitle'); }}
                />
            </div>
        </section>
    );
