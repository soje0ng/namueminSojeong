...
import { useWidgetStyle, getElementStyle, SafeHtml, WidgetRendererProps, formatUnit, UniversalMedia } from './WidgetUtils';
...
export const TEXT_SECTION_DEFAULTS = {
    ...
    blocks: [
        { id: 'b1', type: 'image', url: '/images/template/like_cat.jpg', style: { objectFit: 'cover' } },
        { id: 'b2', type: 'text', text: "텍스트를 더블클릭해서 마음껏 바꿔보세요! 기획부터 꼼꼼한 유지보수까지, 라이크웹이 늘 곁에서 함께할게요.", style: { fontSize: "18px" } },
        { id: 'b3', type: 'heading', text: "라이크웹의 프로젝트명", iconType: 'check', style: { fontSize: "24px", fontSizeMobile: "20px", fontWeight: "600" } },
        { id: 'b4', type: 'text', text: "여기서부터 당신의 이야기가 시작돼요. ✨ 기획부터 완성 후 관리까지 모든 과정을 라이크웹이 친절하게 가이드해 드립니다. 더블클릭 한 번으로 문구를 직접 바꿔보세요! 기획에서 유지보수까지 고민할 필요 없도록, 라이크웹이 알아서 다 해드릴게요.", style: { fontSize: "18px" } }
    ]
};
...
export const TextSectionRenderer: React.FC<WidgetRendererProps> = ({ widget, onElementSelect, viewport = 'desktop' }) => {
    ...
    const renderBlock = (item: any) => {
        ...
        switch (item.type) {
                    ...
                    <div key={item.id} className="group">
                        <div className="flex flex-row items-center gap-[4px] md:gap-[8px]">
                            ...
                            <SafeHtml
                                ...
                                style={blockStyle}
                                onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('blockText', item.id); }}
                            />
                        </div>
                    </div>
                );
                ...
                return (
                    ...
                    >
                        <UniversalMedia
                            ...
                            style={blockStyle}
                            onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('blockUrl', item.id); }}
                        />
            ...
            case 'video':
                return (
                    <div key={item.id} className="relative aspect-[16/9] overflow-hidden shadow-sm group">
                        <UniversalMedia
        ...
        return (
            <section style={style} className="w-full h-auto">
                <div className="mx-auto w-full max-w-[1920px]">
                    <div>
                        <UniversalMedia
                            url={w.data.topImage || "https://selenaimin.likeweb.co.kr/images/banner_img3.jpg"}
                            alt="banner"
                            className="w-full object-cover cursor-pointer"
                            onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('topImage'); }}
                        />
                    </div>
                    <div className="-mt-[20px] flex flex-col gap-[16px] md:-mt-[60px] xl:-mt-[140px]" style={{ gap: w.style?.gap ? formatUnit(w.style.gap) : undefined }}>
                        {(w.data.items || []).map(item => (
                            <div key={item.id} className="flex flex-col gap-[16px] rounded-[30px] bg-[rgba(255,255,255,0.80)] p-[20px] shadow-[0px_4px_10px_0px_rgba(0,0,0,0.10)] backdrop-blur md:gap-[24px] md:p-[24px] xl:flex-row xl:items-center xl:p-[40px]">
                                <div className="flex flex-col items-start gap-[4px] md:flex-row md:gap-[20px] xl:w-1/3">
                                    <SafeHtml
                                        ...
                                        onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('itemNumber', item.id); }}
                                    />
                                    <SafeHtml
                                        html={item.title}
                                        ...
                                        onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('itemTitle', item.id); }}
                                    />
                                </div>
                                <div className="xl:w-2/3">
                                    <SafeHtml
                                        ...
                                        onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('itemSubTitle', item.id); }}
                                    />
                                    <SafeHtml
                                        html={item.desc}
                                        ...
                                        onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('itemDesc', item.id); }}
                                    />
                                </div>
                            </div>
                        ))}
    ...
    if (w.data.variant === 'text3') {
        return (
            <section style={style} className="w-full h-auto">
                <div className="mx-auto w-full max-w-[1920px]">
                    <div className={`flex flex-col gap-[16px] ${viewport === 'desktop' ? 'xl:flex-row xl:items-start xl:gap-0' : 'md:gap-[24px]'}`}>
                        <WidgetHeader
                            title={w.data.title}
                        ...
                        {/* Right Content Area */}
                        <div className="flex flex-col gap-[24px] xl:flex-1 xl:gap-[24px]" style={{ gap: w.style?.gap ? formatUnit(w.style.gap) : undefined }}>
                            {(w.data.items || []).map(item => (
                                <div key={item.id} className="flex flex-col gap-[16px] border-t border-[#666] py-[24px] md:flex-row md:gap-[24px]">
                                    <div
                                        className="flex size-[100px] items-center justify-center rounded-full md:size-[120px] shrink-0 overflow-hidden cursor-pointer"
                                    >
                                        <UniversalMedia
                                            ...
                                            onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('itemIcon', item.id); }}
                                        />
                                    </div>
    ...
    return (
        <section style={style} className="w-full h-auto">
            <div className="mx-auto w-full max-w-[1920px] text-center">
                <div className={`flex flex-col gap-[16px] md:gap-[24px] ${w.data.variant === 'sticky-left' && viewport === 'desktop' ? 'xl:flex-row xl:items-start xl:gap-0' : ''}`}>
                    <WidgetHeader
                        title={w.data.title}
                        subTitle={w.data.subTitle}
                        titleStyle={w.data.titleStyle}
