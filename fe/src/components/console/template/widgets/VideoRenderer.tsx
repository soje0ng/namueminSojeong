...
export const VideoRenderer: React.FC<WidgetRendererProps> = ({ widget, onElementSelect, viewport = 'desktop' }) => {
    ...
    if (isDesktop) {
        const colMap: { [key: number]: string } = { 1: 'grid-cols-1', 2: 'grid-cols-2', 3: 'grid-cols-3', 4: 'grid-cols-4' };
    ...
    return (
        <section style={style} className="w-full h-auto">
            <div className="mx-auto w-full max-w-[1920px]">
                <div className={`flex gap-[0px] md:gap-[0px] ${isSideBySide && isDesktop ? 'flex-row items-start' : 'flex-col'} ${!isSideBySide ? 'items-center text-center' : ''}`}>
                    <WidgetHeader
                        title={w.data.title}
                            ...
                            <div key={item.id || idx} className="flex flex-col gap-4">
                                <div className="relative aspect-video w-full overflow-hidden group">
                                    <UniversalMedia
                                        ...
                                        alt={item.title}
                                        onDoubleClick={onElementSelect ? (e) => { e.stopPropagation(); onElementSelect('url', item.id); } : undefined}
                                    />
                                </div>
                                <div className="text-left">
                                    <SafeHtml
                                        ...
                                        style={getElementStyle({ ...w.data.itemTitleStyle, ...item.itemTitleStyle }, viewport as any)}
                                    ...
                                    <div className="mt-1">
                                        <SafeHtml
                                            ...
                                            style={getElementStyle({ ...w.data.itemDescStyle, ...item.itemDescStyle }, viewport as any)}
