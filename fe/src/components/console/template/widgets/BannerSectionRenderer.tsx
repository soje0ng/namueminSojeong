...
import { useWidgetStyle, getElementStyle, SafeHtml, WidgetRendererProps, UniversalMedia, formatUnit } from './WidgetUtils';
...
export const BANNER_SECTION_DEFAULTS = {
    variant: 'banner1',
    title: "배너 타이틀",
    titleStyle: { fontSize: "32px", fontSizeMobile: "24px", fontWeight: "600", color: "#ffffff" },
    ...
    items: [
        { id: '1', image: "/images/template/img1.png", imageStyle: { borderRadius: "0px" }, link: "#" },
        { id: '2', image: "/images/template/img2.png", imageStyle: { borderRadius: "0px" }, link: "#" },
        { id: '3', image: "/images/template/img3.png", imageStyle: { borderRadius: "0px" }, link: "#" },
        { id: '4', image: "/images/template/img4.png", imageStyle: { borderRadius: "0px" }, link: "#" },
        { id: '5', image: "/images/template/img5.png", imageStyle: { borderRadius: "0px" }, link: "#" },
        { id: '6', image: "/images/template/img6.png", imageStyle: { borderRadius: "0px" }, link: "#" },
        { id: '7', image: "/images/template/img7.png", imageStyle: { borderRadius: "0px" }, link: "#" },
        { id: '8', image: "/images/template/img8.png", imageStyle: { borderRadius: "0px" }, link: "#" },
    ]
};
    ...
    viewport
        ...
        return (
            <a
                ...
                className={`flex items-center justify-between p-[12px_16px] md:p-[16px_24px] hover:shadow-lg transition-all cursor-pointer hover:outline-dashed hover:outline-2 hover:outline-blue-400 ${viewport === 'mobile' ? 'w-full' : 'flex-1'}${viewport === 'desktop' ? ' xl:max-w-[280px]' : ''}${!item.textStyle?.backgroundColor ? ' bg-white' : ''}${!item.textStyle?.borderRadius ? ' rounded-full' : ''}`}
                style={style}
                onClick={(e) => {
                    if (onElementSelect) {
                        e.preventDefault();
            ...
            >
                ...
                />
                <span
                    className="material-symbols-outlined ml-2 text-[20px] md:text-[24px]"
                >
                    {item.icon || 'download'}
                </span>
            </a>
...
export const BannerSectionRenderer: React.FC<WidgetRendererProps> = ({ widget, onElementSelect, viewport = 'desktop' }) => {
    ...
    const { backgroundColor, backgroundImage, backgroundSize, backgroundPosition, backgroundRepeat, ...style } = _style;
    ...
    if (w.data.variant === 'banner1') {
        return (
            <section style={style} className="w-full h-auto">
                <div className="mx-auto w-full max-w-[1920px]">
                    <div
                        className="p-[24px_20px] text-center rounded-[30px] md:p-[40px_24px]"
                        style={{
                    ...
                    >
                        ...
                        <div className="mt-[8px]">
                            <SafeHtml
                                ...
                                style={descStyle}
                                onDoubleClick={(e) => { e.stopPropagation(); onElementSelect?.('desc'); }}
                            />
                        </div>
                        <div
                            ...
                            {(w.data.items || []).map(item => (
                                ...
                                >
                                    <UniversalMedia
                                        ...
                                        onDoubleClick={onElementSelect ? (e) => { e.stopPropagation(); onElementSelect('itemImage', item.id); } : undefined}
                                    />
                                    {item.link && (
                                        <a
                                            href={item.link}
        ...
        if ((b2TitleStyle.color === '#ffffff' || b2TitleStyle.color === '#fff' || !b2TitleStyle.color) && !w.style?.backgroundImage && (!w.style?.backgroundColor || w.style.backgroundColor === '#F8F6F2')) {
        ...
        return (
            <section style={style} className="w-full h-auto">
                <div className="mx-auto w-full max-w-[1920px]">
                    <div
                        className={`flex flex-col gap-[24px] p-[24px_20px] rounded-[30px] md:p-[40px_24px] ${viewport === 'desktop' ? 'xl:flex-row xl:items-center xl:justify-between xl:p-[40px_60px]' : ''}`}
                        style={{
                            ...(w.style?.backgroundImage ? {
                                backgroundImage: `url(${w.style.backgroundImage})`,
                                backgroundSize: 'cover',
                    ...
                    >
                        ...
                        <div className="md:min-w-[280px]">
                            <BannerButton
                                item={w.data.items?.[0] || { id: 'temp', link: '#', icon: 'download' }}
        ...
        return (
            <section style={style} className="w-full h-auto">
                <div className="mx-auto w-full max-w-[1920px]">
                    <div
                        className={`relative flex flex-col gap-[24px] p-[24px_20px] rounded-[30px] md:p-[40px_24px] ${viewport === 'desktop' ? 'xl:p-[40px_60px_40px_500px]' : ''} ${(!w.style?.backgroundImage && !w.style?.backgroundColor) ? 'bg-gradient-to-r from-[#21568E] to-[#093666]' : ''}`}
                        style={{
                            ...(w.style?.backgroundImage ? {
                                backgroundImage: `url(${w.style.backgroundImage})`,
                                backgroundSize: 'cover',
                    ...
                    >
                        {/* Mascot Image/Video */}
                        <div className={`absolute bottom-0 right-0 hidden aspect-[6/7] max-w-[240px] hover:outline-dashed hover:outline-2 hover:outline-blue-400${viewport !== 'mobile' ? ' md:block' : ''}${viewport === 'desktop' ? ' xl:left-[240px] xl:right-auto' : ''}`}>
                            <UniversalMedia
                                ...
                                onDoubleClick={onElementSelect ? (e) => { e.stopPropagation(); onElementSelect('image'); } : undefined}
                            />
                        </div>
