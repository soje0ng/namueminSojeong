...
export type Section9Type = "text" | "features" | "basicText";

export interface Section9FeatItem {
  id: string;
  title: string;
  desc: string;
  iconUrl?: string;
...
const SECTION_LABELS: Record<Section9Type, string> = {
  text: "텍스트 영역",
  features: "프로그램 특징",
  basicText: "기본 텍스트",
};

const SECTION_COLORS: Record<Section9Type, string> = {
  text: "bg-gray-100 text-gray-700",
  features: "bg-orange-100 text-orange-700",
  basicText: "bg-green-100 text-green-700",
};

function createDefaultSection(type: Section9Type): Section9Item {
  const id = `s9-${type}-${Date.now()}`;
  if (type === "text") {
    return {
      id,
      type: "text",
...
const TextStructure9Manager: React.FC<Props> = ({
  ...
  const updateSection = (id: string, patch: Partial<Section9Item>) => {
  ...
  const addSection = (type: Section9Type) => {
      ...
      <div className="relative">
        ...
        {showAddPicker && (
          <div className="absolute bottom-full left-0 right-0 mb-1.5 bg-white rounded-xl shadow-lg border border-gray-200 p-2 z-10 grid grid-cols-2 gap-1.5">
            {(["text", "features", "basicText"] as Section9Type[]).map(
              (type) => (
                <button
                  key={type}
...
export default TextStructure9Manager;
