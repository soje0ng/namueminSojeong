...
export const PropertyPanel: React.FC<PropertyPanelProps> = ({
  ...
  const handleItemDrop = (arrayName: string) => {
    if (
      ...
      const draggedId = items[itemDraggedIdx]?.id;
      const targetId = items[itemDragOverIdx]?.id;
  ...
  if (
    ...
    ["6", "7"].includes((widget.data as any).layout || "1")
  )
    listArrayName = null;
                  ...
                  {widget.type === "table" ? (
                    ...
                    <>
                      ...
                      <div className="space-y-1 max-h-[250px] overflow-y-auto border rounded p-2 bg-gray-50 mt-2">
                        {((widget.data as any)[listArrayName!] || []).map(
                          (item: any, idx: number) => {
                            const items =
                              (widget.data as any)[listArrayName!] || [];
                            const itemTextProp =
