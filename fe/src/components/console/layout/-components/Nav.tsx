...
export default function Nav() {
    ...
    useEffect(() => {
        ...
        if (!loginUser?.m_menu_auth || typeof loginUser.m_menu_auth !== 'string') {
            // 권한이 null이거나 문자열이 아니면 모든 메뉴 표시
            setMenuList(baseMenuList);
            return;
        }

        const authArray = loginUser.m_menu_auth.split(",").map(auth => parseInt(auth.trim())).filter(num => !isNaN(num));
                ...
                <ScrollArea className="flex-1">
                    <ul className="pr-[16px]">
                        {menuList.map((menu, i) => (
                            <li
                                key={`menu_${i}`}
                                className={`relative rounded-[0_16px_16px_0] after:absolute after:inset-0 after:h-full after:w-[4px] after:bg-console after:opacity-0 after:transition-all after:duration-300${menuOn?.includes(menu.menuId) ? " bg-[#F8F9FB] after:opacity-100" : ""
                                    }`}
                            >
                                <button
                                    type="button"
                                    className={`flex w-full items-center justify-between p-[17px_0_17px_16px] text-[18px] text-[#9F9FA5]${menuOn?.includes(menu.menuId) ? " [&>p]:font-[700] [&>p]:text-console" : ""
                                        }`}
                                    onClick={() => {
                                        if (menu.subItems) {
                                ...
                                {menu.subItems && (
                                    <ul
                                        className={`flex flex-col gap-[6px] overflow-hidden ${menuOn?.includes(menu.menuId)
                                    ...
                                    >
                                        {menu.subItems.map((subMenu, j) => (
                                            <li key={`sub_menu_${j}`}>
                                                <button
                                                    type="button"
                                                    className={`flex w-full items-center justify-between p-[6px_0_6px_60px] text-left text-[#666]${menuOn === subMenu.id ? " underline" : ""
                                                        }`}
                                                    onClick={() => {
                                                        if (subMenu.subItems) {
                                                ...
                                                {subMenu.subItems && (
                                                    <ul
                                                        className={`overflow-hidden ${menuOn?.includes(subMenu.id)
                                                        ...
                                                        {subMenu.subItems.map((subMenu2, k) => (
                                                            <li key={`sub_menu2_${k}`}>
                                                                <Link
                                                                    ...
                                                                    className={`block truncate p-[6px_6px_6px_60px] text-[14px] font-[500] text-[#878b93]${menuOn === subMenu2.id ? " underline" : ""
                                                                        }`}
                                                                >
                                                                    {subMenu2.label}
                                                                </Link>
