...
import TemplatePop from "@/components/console/menu/category/-components/TemplatePop";
import type { PageData } from "@/types/console/template";
                ...
                <form onSubmit={handleSubmit(handleConfirmSave)} className="flex h-full flex-col">
                    ...
                    <div className="min-h-0 flex-1 border-t border-[#D9D9D9]">
                        <ScrollArea className="h-full">
                            <ul className="flex flex-col gap-[20px] p-[20px_40px] min-h-full">
                                ...
                                {isSub && ( // 하위 카테고리일때만 노출
                                    <li className="flex flex-1 flex-col gap-[8px]">
                                        <p className="text-[#666]">카테고리 종류</p>
                                        <Tabs list={tabList} activeIdx={tabOn} handleClick={handleChangeTab} />
                                        <div className="flex-1 overflow-hidden rounded-[8px] border border-[#D9D9D9]">
                                            ...
                                            <ul className="flex flex-wrap gap-[20px] border-t border-[#D9D9D9] p-[20px]">
                                                {tabOn === 0 && ( // HTML 카테고리일때만 노출
                                                    <li className="flex w-full flex-col gap-[8px]">
                                                        <TemplatePop
                                                            onSave={(htmlCode, pageData) => {
                                                                setValue("content", htmlCode);
                                                                setValue("page_data", JSON.stringify(pageData));
                                                            }}
                                                            initialPageData={(() => {
                                                                if (!values.page_data) return undefined;
                                                                try {
                                                                    return JSON.parse(values.page_data) as PageData;
                                                                } catch {
                                                                    return undefined;
                                                                }
                                                            })()}
