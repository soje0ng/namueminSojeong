...
export default function LanguageTabs({ activeLang, handleLanguageChange }: LanguageTabsProps) {
    const { siteLanguages, _hasHydrated } = useSiteStore();
    const liStyle =
    ...
    // 하이드레이션 전이거나 siteLanguages가 없거나 빈 배열인 경우 처리
    if (!_hasHydrated || !siteLanguages || !Array.isArray(siteLanguages) || siteLanguages.length === 0) {
        return null;
    }
