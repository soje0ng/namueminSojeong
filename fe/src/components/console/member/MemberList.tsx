...
function MemberList() {
    ...
    // currentPage 변경 시 URL 파라미터 업데이트 (memoized)
    const handleChangeCurrentPage = useCallback(
        ...
        [updateUrlParams, setCurrentPage],
    );
    ...
    const handleSearch = useCallback(() => {
        ...
        setCurrentPage(1);
    }, [searchFilterValues, updateUrlParams, setCurrentPage]);
    ...
    const handleOpenDetail = useCallback(
        ...
        [detailOn, urlParams, updateUrlParams],
    );
    ...
    }, [checkedList, level, putMemberLevelMutation, toast]);

    // 회원등급 변경 확인 (memoized)
    const handleConfirmLevelChange = useCallback(() => {
    ...
    }, [checkedList.length, level, setConfirmPop, handleLevelChange]);
    ...
    }, [checkedList, detailOn, urlParams, items.length, delMemberMutation, toast, updateUrlParams, setCurrentPage]);
    ...
    }, [checkedList.length, setConfirmPop, handleDelete]);

    // 회원 정보수정 취소시 (memoized)
    ...
    const onDeleteComplete = useCallback(() => {
        ...
        setCurrentPage(prevPage);
    }, [urlParams, items.length, updateUrlParams, setCurrentPage]);
