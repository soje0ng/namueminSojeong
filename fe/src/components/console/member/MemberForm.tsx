...
function MemberForm({ detailIdx, handleCancel, onDeleteComplete }: MemberFormProps) {
    const {
        handleSubmit,
        formState: { errors },
        reset,
    ...
    // 관리자권한 체크시 (memoized)
    const handleChangeMenuAuth = useCallback(
        (checked: boolean, authCode: string, currentValue: string[]) => {
            if (checked) {
    ...
    }, [detailIdx, delMemberMutation, toast, onDeleteComplete]);
